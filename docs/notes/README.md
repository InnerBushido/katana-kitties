...
| [endgame.md](endgame.md) | the 100% ending, the Awakening, the eight Powerup Kotodama, the economy, the trade screen, the Cross Slash rebalance and the tell that warns you it is coming, **the last-five countdown and the map mark that ends the hunt**, **and the balance page** | `systems/kotodama.js`, `systems/lasthunt.js`, `systems/profile.js`, `entities/powerorb.js`, `tuning.html`, `src/core/tuning.js` |
...
| [docs.md](docs.md) | **the God Doc's own rules** — when a change earns a line in PROJECT.md and when it does not, how §5 and §6 generate themselves out of the code, and the three places that is enforced | `tools/doc-sync.mjs`, `.githooks/pre-commit`, anything that changes the inputs or the balance numbers, adding a tool or a note |
