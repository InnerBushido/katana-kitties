...
**Keep this file current — that is the rule, and it is not optional.** Anything
a new developer would need to know and could **not** rediscover by reading the
code gets a line here, in the section it belongs to, pointing somewhere deeper:

- a new **tool, document, account, service or cost**;
- a new way the project is **built, tested or shipped** — the `alpha` channel in
  §3 is exactly this: nothing about it lives in `src/`, so nothing in `src/`
  could ever have told anybody it exists;
- a change to the **inputs** or to the **numbers this file quotes** — that one is
  automated rather than remembered, see below;
- a new **rule that must not break**, which goes in §9 *and* in [CLAUDE.md](CLAUDE.md).

A new system, a refactor, a bug fix: **no line.** `git log` is the session log
and the design notes carry the reasoning. The test is only ever *"would somebody
find this out by reading the code?"*

**Two of the tables below write themselves.** §5's balance numbers and §6's
controls are generated out of the source by `npm run docs`, re-generated by a
commit hook, and checked by `world-check` — because a restatement kept in step
by hand is kept in step until the first busy session. Everything outside the
`<!-- doc-sync -->` markers is human prose and is never touched.

**And the published page is a second copy that goes stale on its own.** Its
source is [docs/artifact/project-page.html](docs/artifact/project-page.html),
the same `npm run docs` writes its tables and its check counts, and
[docs/artifact/published.json](docs/artifact/published.json) holds the hash of
what was last published so `world-check` can fail when it falls behind. It had
drifted three revisions and one wrong fact before anybody looked. **Republishing
is not re-sharing** — people already holding the link stay pinned to the version
they were given until the share pin is moved. → [docs.md](docs/notes/docs.md)

**Everything else is enforced too.** `world-check` reads this file and fails if a
design note is unlinked, a runnable tool unnamed, a check total disagrees with
what the script prints, or the **Last updated** line above is missing.
**This file is the version to edit** — the published page is generated from it,
so re-publish after a change rather than editing the page.
...
npm run artifact              # tools/artifact-sync.mjs — is the published page current?
```
...
### Anywhere at all — the alpha channel

**`alpha` is the testing channel.** It is an ordinary long-lived branch with one
job: it is the version other people are asked to play. Push to it and Vercel
builds it within a minute at a **stable** address that never changes:

```
https://katana-kitties-git-alpha-dream-dojo.vercel.app
```

That link is the whole channel. There is no Discord, no TestFlight, no Steam
beta — a tester opens the URL, or installs it to their phone's home screen and
plays it fullscreen. **`main` is what the girls play and is not for testing.**

```bash
git checkout alpha && git merge main --ff-only    # alpha only ever follows main
git push origin alpha
```

**Alpha follows `main`, it does not lead it.** Work lands on a typed branch, is
merged into local `main`, and `alpha` fast-forwards to it — so `alpha` is always
a commit that exists on `main` and there is never a fix on the testing channel
that the real game has not had. **`origin/main` is still not pushed until
Richard has played it**; `alpha` is the step before that, not a way around it.

**Every other branch gets the same treatment**, at the same shape of alias — so
a single risky change can be handed to somebody without going near `alpha`:
...
**Tell a tester that**, or the first thing they report is having lost everything.
...
deletes either is exactly what non-negotiable 7 is about. Both ask first and
both put the count in the YES button. There is no other way to clear the record
board from inside the game — the only alternative is the browser console:
`Object.keys(localStorage).filter(k => k.startsWith('kk.arena.board')).forEach(k => localStorage.removeItem(k))`.
...
**This table is generated out of `src/entities/player.js` and is never edited by
hand** — `npm run docs` writes it, a commit hook rewrites it, and `world-check`
fails if it has drifted. Everything marked ✎ is on the balance page, and that
mark is derived too: it means the number was read out of a `tune()` table, so a
✎ can never appear beside a number the page cannot reach.
→ [docs.md](docs/notes/docs.md)
...
for 0.8s — a head-back inhale pose, a vortex of specks drawn into her mouth, a
...
wherever the stick points for a full second, **walking and turning as she goes**:
the cone is a live hitbox for all of it, so sweeping it across the ring catches
everybody it crosses, and catches each of them exactly once. It cannot be
...
Joy-Con button names were wrong in the game for months and nothing anywhere
would have noticed this page still naming the old ones.
→ [docs.md](docs/notes/docs.md)
...
| jump | `Space` | `Numpad0` `RAlt` `RCtrl` |
| attack | `F` | `Numpad1` `/` `J` |
| interact | `E` | `Numpad2` `,` `I` |
| mount | `Q` | `Numpad3` `.` `P` |
...
<!-- doc-sync:pads -->
| action | Xbox | PlayStation | Joy-Con L | Joy-Con R |
| --- | --- | --- | --- | --- |
...
| map zoom | **RB** | **R1** | **L** | **R** |
| maths overlay | **LB** | **L1** | **ZL** | **ZR** |
<!-- /doc-sync:pads -->

The two Joy-Con halves are separate columns because they are separate
controllers — held sideways, each with its own four-button cluster, and the two
clusters are rotated differently in the driver. The left half's names are
**directions as she holds it**, since a d-pad has no letters to appeal to.
...
the check that would have caught it.** That is why `world-check` is 3456
assertions.
...
tested or shipped. Not a refactor and not a bug fix: the test is whether anybody
could find it out by reading the code. The inputs and the numbers in §5 and §6
are handled by machine instead of by memory. → [docs.md](docs/notes/docs.md)

**Git:** typed branches (`feature/`, `bugfix/`, `mixed/`), a `commit-msg` hook
that stamps `[branch]` on non-main commits, a `pre-commit` hook that regenerates
this file's tables, merged locally, and **never pushed to `origin/main` until
Richard has played it**. Branches are kept until pushed, then `git branch -d`
(never `-D`). **`alpha` is the testing channel** and only ever fast-forwards to
`main` — §3.
...
| [docs.md](docs/notes/docs.md) | **this file's own rules** — when a change earns a line here, how §5 and §6 generate themselves, and how the published twin is kept in step |
...
project than netcode, independently useful, and the half players would actually
notice. `systems/leaderboard.js` and `systems/savegame.js` both already have the
shape; pointing them at a real table is a small change behind the same
interfaces. Then national boards (and the moderation question a public board with
