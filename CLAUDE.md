...
| how a cutscene, camera move or recorded clip is directed and checked | the `game-cutscene-director` skill, [.claude/skills/](.claude/skills/game-cutscene-director/SKILL.md) — loads itself when the work is a scene |
...
src/
  ...
  systems/   tournament  menagerie  arenaquest  leaderboard
             ...
             savegame (the afternoon, written down every 30s once a game is
               ...
               who swore where — and never the world, which rebuilds itself
               from its own seeds; so a save is 2KB and `worldSig` refuses one
               taken in a build with a different prop count rather than putting
               the wrong barrels down. ONE ROW PER PLAY SESSION: `Game.sessionId`
               is stamped into every snapshot and `putSave` replaces the row
               carrying it, so five slots are five GAMES — before that, five
               slots filled with one afternoon in three minutes. And a row holds
             ...
             clanfx (the same trick again, for the two clan powers: Steal
               Mischief's mark ring and the Dragon Breath's charge and cone)
  ...
  entities/  player  dragon  ryuuseki  panda  critter  angel  leader  satan
             ...
             clanpower (what an OATH is worth in the ring — 盗 Steal Mischief
               and 息 Dragon Breath, both on ACTION, both tune()d. The steal is
               a MARK paid by an ordinary gated hit, not a second combat path;
               the breath is a row in ATTACKS. Non-negotiable 3 is intact.
               docs/notes/tournament.md)
tools/       world-check.mjs  pad-check.mjs  png.mjs (dependency-free codec)
  ...
  capture/   the rig that FILMS the game to make those clips — a shot script
             ...
             **tools/capture/README.md is the director's guide** and is the
             ...
             satan-countdown.mjs (cuts the five clips the last fifteen
               seconds of a round are made of, from the takes kept in
               `satan-takes/`. The count is ONE continuous performance
               re-timed, not nine takes assembled: the words are found by
...
- **When you fix something, add the check that would have caught it.** That is
  why `world-check` is 3503 assertions and why almost none of them are about
...
- **Before designing a menu, dialog or any list, read "UI FALL-THROUGH" in
  ...
  re-inventing — a press acted on twice, an edge manufactured by a device
  changing hands, or a row that slid under the cursor after she aimed at it —
  and it has been found five times in five places that look nothing alike.
